...
export interface TransitContextType {
  ...
  currentUserName: string; // SECURITY: Display user name to avoid role confusion
  isOffline: boolean;
  isLocked: boolean; // SECURITY: Session verrouillée (mise en veille/inactivité)
  toggleOffline: () => void;
  shipments: Shipment[];
  ...
  quickUnlock: (password?: string) => Promise<boolean>; // SECURITY: Déverrouillage rapide
  lockSession: () => void; // SECURITY: Verrouillage manuel
  /**
